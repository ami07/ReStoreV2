...
public class AccumulativeBag implements DataBag {
     ...
     */
	@Override
	public boolean isEquivalent(DataBag bag) {
		if(bag instanceof AccumulativeBag){
			if(buffer.isEquivalent(((AccumulativeBag) bag).buffer) && index == ((AccumulativeBag) bag).index){
				return true;
			}
		}
		return false;
