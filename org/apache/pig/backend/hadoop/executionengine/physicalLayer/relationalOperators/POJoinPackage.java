...
public class POJoinPackage extends POPackage {
	...
	public boolean isEquivalent(PhysicalOperator otherOP) {
		if(otherOP instanceof POJoinPackage){
			if(isEquivalentPackageComponents((POJoinPackage)otherOP) &&  isEquivalentForEachComponents((POJoinPackage)otherOP)){
				return true;
			}
		}
		return false;
	 ...
	 * @author iman
	 */
	private boolean isEquivalentForEachComponents(POJoinPackage otherOP) {
		if(forEach.isEquivalent(otherOP.getForEach())){
			return true;
		}
		return false;
	}
	 ...
	 * @author iman
	 */
	private boolean isEquivalentPackageComponents(POJoinPackage otherOP) {
		if(super.isEquivalent(otherOP)&& isKeyTuple==otherOP.isKeyTuple){
			if(keyInfo!=null && otherOP.keyInfo!=null && keyInfo.size() == otherOP.keyInfo.size()){
				//for(true)
				return true;
			}else if(keyInfo==null && otherOP.keyInfo==null ){
