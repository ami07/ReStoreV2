...
public class MapReduceLauncher extends Launcher{
    ...
    public PigStats launchPig(PhysicalPlan php,
                              ...
                              PigContext pc) throws PlanException,
                                                    ...
                                                    Exception {
    	long pigLauncherStartTime=System.currentTimeMillis();
        long sleepTime = 500;
        ...
        //@iman: find sharing opportunities with the stored tmp results
        //QuerySharingOptimizer qsOptimizer = new QuerySharingOptimizer(pc, conf);
        //qsOptimizer.discoverSharinpOpportunities();
        
        //read the properties about whether or not we want to find sharing opportunities
        boolean isOptimizeBySharing=conf.getBoolean(OPTIMIZE_BY_SHARING, false);
        ...
        while(mrp.size() != 0) {
            ...
            if(jobsWithoutIds.size()>0){//get job tracker url only if there are jobs to be executed
            ...
            while(!jc.allFinished()){
            	...
            	for(Job job : jobsWithoutIds){
            		if (job.getAssignedJobID() != null){
            			...
            			//@iman:add ana entry in the job id map
            			jcJobIdMap.put(job.getAssignedJobID().toString(),job.getJobID());
            			jcJobIdMapReverse.put(job.getJobID(),job.getAssignedJobID().toString());
            ...
            //@iman: get the jobs execution time
            //Map<String, Long> jobExecTimes = jc.getJobsExecTime();
            //jc.getJobsExecTime();
            //log.info("Job execution times: "+jobExecTimes);
            
            jcc.copyResultsToSharedLocation(jobs);
            ...
            //@iman: update the stats associated with the job we have just executed to decide later
            //if we are gonna keep it
            /*PigStats ps=PigStatsUtil.getPigStats(0);
            JobGraph jGraph = ps.getJobGraph();
            Iterator<JobStats> iter = jGraph.iterator();
            while (iter.hasNext()) {
                JobStats js = iter.next();
                String jsJobId=js.getJobId();
                String jsID=jcJobIdMap.get(jsJobId);
                jcc.updateCandidatePlanStats(jsID,js,0l/*jobExecTimes.get(job.getJobID())*);
            }*/
            //Map<String, Map<String, String>> allJobStats = ps.getP//PigStatsUtil.getPigStats(0).getPigStats();//.g.getPigStats();
            //for(Job job : jc.getSuccessfulJobs()) {
	        //    Map<String, String> jobStats = allJobStats.get(job.getAssignedJobID().toString());
	        //    System.out.println("To update plan stats for job: "+job.getJobID()+"  and assigned job id "+job.getAssignedJobID().toString());
	        //    jcc.updateCandidatePlanStats(job.getJobID(),jobStats,jobExecTimes.get(job.getJobID()));
            //}
        ...
        //@iman: finished running all jobs, dump the plans that we think are useful
        //jcc.dumpSharedPlans();
        ...
        if (succJobs != null) {
            for (Job job : succJobs) {
                ...
                //@iman: if this job has its input available (not temp files) then consider storing it as a view
                ...
                if(isOptimizeBySharing){
	                ...
	                while (iter.hasNext()) {
	                    JobStats js = iter.next();
	                    String jsJobId=js.getJobId();
	                    String thisJobAssignedId=job.getAssignedJobID().toString();
	                    //System.out.println("jsJobId="+jsJobId+"  thisJobAssignedId"+thisJobAssignedId);
	                    if(jsJobId!=null && jsJobId.equals(thisJobAssignedId)){
		                    String jsID=jcJobIdMap.get(jsJobId);
		                    jcc.updateCandidatePlanStats(jsID,js,0l,jGraph,jcJobIdMapReverse/*jobExecTimes.get(job.getJobID())*/);
		                    break;
	                    }
	                }
                }
                
                //@iman: else combine it with its previous dependent jobs and consider storing the final result as a view!!
        ...
        //@iman: dump the new plans that we have looked at when executing this query/script
        if(isOptimizeBySharing){
        	jcc.dumpSharedPlans();
        }
        ...
        long pigLauncherEndTime=System.currentTimeMillis();
        long pigLauncherTotalTime=pigLauncherEndTime-pigLauncherStartTime;
        log.info("Total pig launcher time "+pigLauncherTotalTime+"  ms");
    ...
    private MROperPlan compile(
        ...
        Configuration conf = ConfigurationUtil.toConfiguration(exe.getConfiguration());
        boolean isMoreDebugInfo=conf.getBoolean(DEBUGINFO_DETAILED, false);
        //boolean isShareOperators=conf.getBoolean(OPTIMIZE_SHARING_OPERATORS, false);
        
        //@iman: print map reduce plans for the query before optimizing it
      //@commByIman
        if(isMoreDebugInfo){
	        boolean verbose = true;
	        PrintStream ps=System.out;
	        ps.println("#--------------------------------------------------");
	        ps.println("# Map Reduce Plan before optimization                             ");
	        ps.println("#--------------------------------------------------");
	        MRPrinter printer = new MRPrinter(ps, plan);
	        printer.setVerbose(verbose);
	        printer.visit();
        }
        ...
        // optimize joins
        boolean isOptimizeJoin = "true".equalsIgnoreCase(pc.getProperties().getProperty("opt.join","true"));
        if(isOptimizeJoin){
        ...
        //@iman
        //QuerySharingOptimizer qsOptimizer = new QuerySharingOptimizer(plan);
       // qsOptimizer.visit();
      ...
      //@commByIman
        if(isMoreDebugInfo){
	        ...
	        PrintStream ps=System.out;
	        ps.println("#--------------------------------------------------");
	        ps.println("# Map Reduce Plan  after optimization and before finiding sharing opportunities                               ");
	        ps.println("#--------------------------------------------------");
	        MRPrinter printer = new MRPrinter(ps, plan);
	        printer.setVerbose(verbose);
	        printer.visit();
        ...
        //@iman: finding sharing opportunities
	...
	public MROperPlan launchPigCompileMRP(PhysicalPlan plan, PigContext pigContext) throws PlanException, VisitorException, IOException {
		MROperPlan mrp = compile(plan, pigContext);
		return mrp;
	}
