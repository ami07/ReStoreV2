...
public final class PigStats {
     ...
     * @author iman
     */
    void updateMRGraph(JobControlCompiler jcc,  MapReduceOper mr,  MapReduceOper mrSucc) {
    	
    	if (jcc == null || !jcc.equals(this.jcc)) {
            LOG.warn("invalid params: " +  jcc);
            return;
        }
        ...
        //connect mr with its successor 
        JobStats jSucc = null;
        if(mrSucc!=null){
        	jSucc = mroJobMap.get(mrSucc);
        }
        if (jSucc!=null && !jobPlan.isConnected(js, jSucc)) {
            jobPlan.connect(js, jSucc);
        }
        
        //add the new jobStats to the mroJobMap
        mroJobMap.put(mr, js);                  
    }
    
    void updateMRGraphReplace(JobControlCompiler jcc,  MapReduceOper oldMRO,  MapReduceOper newMRO,List<MapReduceOper> mrSuccs) throws FrontendException {
    	if (jcc == null || !jcc.equals(this.jcc)) {
            LOG.warn("invalid params: " +  jcc);
            return;
        }
    	...
    	JobStats js = new JobStats(
    			newMRO.getOperatorKey().toString(), jobPlan);  
    	
    	//get the old job stat or oldMRO
    	JobStats oldJs = mroJobMap.get(oldMRO);
    	
    	//replace the old js with the new js
    	if(oldJs!=null){
    		jobPlan.replace(oldJs, js);
    	}
    	
    	//check that any succ exist and therefore connect them if they are not already connected
    	if(mrSuccs!=null){
	    	for(MapReduceOper mrSucc:mrSuccs){
	    		JobStats jSucc = null;
	            if(mrSucc!=null){
	            	jSucc = mroJobMap.get(mrSucc);
	            }
    	...
    	//update the mroJobMap
    	mroJobMap.put(newMRO, js);
    	
    	//remove job and mro from mroJobMap graph
    	if(oldJs!=null && mroJobMap.containsValue(oldJs)){
    		Set<MapReduceOper> mrKeys = mroJobMap.keySet();
    		MapReduceOper targetMR=null;
    		for(MapReduceOper mr:mrKeys){
    			if(mroJobMap.get(mr).equals(oldJs)){
    				targetMR=mr;
    				break;
    			}
    		}
    		if(targetMR!=null){
    			mroJobMap.remove(targetMR);
    		}
    	}
    }
    void updateMRGraphRemoveJob(JobControlCompiler jcc,  JobStats js) throws FrontendException{
    	if (jcc == null || !jcc.equals(this.jcc)) {
            LOG.warn("invalid params: " +  jcc);
            return;
        }
    	
    	//remove job from jobPlan
    	if(js!=null){
    		jobPlan.remove(js);
    	}
    	//remove job and mro from mroJobMap graph
    	if(js!=null && mroJobMap.containsValue(js)){
    		Set<MapReduceOper> mrKeys = mroJobMap.keySet();
    		MapReduceOper targetMR=null;
    		for(MapReduceOper mr:mrKeys){
    			if(mroJobMap.get(mr).equals(js)){
    				targetMR=mr;
    				break;
    			}
    		}
    ...
    public JobStats getJobForMRO(MapReduceOper mro){
    	if(mro!=null){
    		return mroJobMap.get(mro);
    	}
    	return null;
    }
