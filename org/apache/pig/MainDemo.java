...
/**
 * Main class for Pig engine, used for demo
 * modified by @author iman
 */
...
private ArrayList<String> paramFiles;

private HashSet<String> optimizerRules;
...
private boolean userSpecifiedLog;
...
String pigQueryFileName;

private MROperPlan mapReducePlan;

private MROperPlan updatedMapReducePlan;

private MultiMap<String, MapReduceOper> discoveredPlans;

private Vector<SharedMapReducePlan> sharedPlans;

private Vector<Boolean> selectedSharedPlans;

public int setupPig(Properties reStoreProperties){
	int rc = 1;
    verbose = false;
    logFileName = null;
    ...
    try {
        ...
        if(reStoreProperties!=null){
        	properties.putAll(reStoreProperties);
        }
        
        String[] pigArgs = parser.getRemainingArgs();
        
        
        userSpecifiedLog = false;
        ...
        paramFiles = new ArrayList<String>();
     ...
     // create the context with the parameter
        ...
        // create the static script state object
        String commandLine = LoadFunc.join((AbstractList<String>)Arrays.asList(args), " ");
        scriptState = ScriptState.start(commandLine);
...
public int runQueryToEnd(){
	if(pigQueryFileName==null){
		return -1;
	}
	int rc=1;
	Grunt grunt = null;
    ...
    try{
		FileLocalizer.FetchFileRet localFileRet = FileLocalizer.fetchFile(properties, pigQueryFileName);
	    if (localFileRet.didFetch) {
	        properties.setProperty("pig.jars.relative.to.dfs", "true");
	    }
	    in = new BufferedReader(new FileReader(localFileRet.file));
	    ...
	    // run parameter substitution preprocessor first
	    substFile = pigQueryFileName + ".substituted";
	    pin = runParamPreprocessor(properties, in, params, paramFiles, substFile, debug || dryrun || checkScriptOnly);
	    if (dryrun) {
	        log.info("Dry run completed. Substituted pig script is at " + substFile);
	        return ReturnCode.SUCCESS;
	    ...
	    logFileName = validateLogFile(logFileName, pigQueryFileName);
	    pigContext.getProperties().setProperty("pig.logfile", logFileName);
	
	    // Set job name based on name of the script
	    pigContext.getProperties().setProperty(PigContext.JOB_NAME, 
	                                           "PigLatin:" +new File(pigQueryFileName).getName()
	    );
	    ...
	    scriptState.setScript(new File(pigQueryFileName));
	    ...
	    if(checkScriptOnly) {
	        grunt.checkScript(substFile);
	        System.err.println(pigQueryFileName + " syntax OK");
	        rc = ReturnCode.SUCCESS;
	    } else {
	        int results[] = grunt.exec();
	        rc = getReturnCodeForStats(results);
...
public int runQueryCompilePP(){
    ...
    try{
	    ...
	    if(checkScriptOnly) {
	        ...
	        //int results[] = grunt.exec();
	    	physicalPlan = grunt.execQueryCompile();
	        //rc = getReturnCodeForStats(results);
...
public int runQueryCompileMRP(){
	int rc=1;
	if(physicalPlan == null || grunt == null){
		return ReturnCode.FAILURE;
	}
	try{
		mapReducePlan = grunt.execQueryCompileMRP();
		
		//change all store names
	...
	//set the updated MR plan to the same MRplan
	this.updatedMapReducePlan = mapReducePlan.clone();
	
	return rc;
}
public MROperPlan runQueryRewriteUsingStoredPlans(Vector<SharedMapReducePlan> sharedPlans, Vector<Boolean> selectedSharedPlans){
	 //Vector<SharedMapReducePlan> sharedPlans = new Vector<SharedMapReducePlan>(); 
	 //Vector<Boolean> selectedSharedPlans = new  Vector<Boolean>();
	 MROperPlan rewrittenMapReducePlan =null;
	 try {
		rewrittenMapReducePlan = MROperPlanRewriteUtils.rewriteWithSharedDefault(mapReducePlan, sharedPlans, selectedSharedPlans, pigContext);
		
		this.sharedPlans=sharedPlans;
		this.selectedSharedPlans=selectedSharedPlans;
		this.updatedMapReducePlan=rewrittenMapReducePlan;
	 ...
	 return rewrittenMapReducePlan;
}

public MROperPlan runQueryRewriteUsingStoredPlans(Vector<SharedMapReducePlan> sharedPlans){
	
	MROperPlan rewrittenMapReducePlan =null;
	try {
		Vector<SharedMapReducePlan> selectedPlans = new Vector<SharedMapReducePlan>();
		rewrittenMapReducePlan = MROperPlanRewriteUtils.rewriteWithShared(mapReducePlan, sharedPlans, selectedPlans , pigContext);
	} catch (VisitorException e) {
		e.printStackTrace();
	} catch (PlanException e) {
		e.printStackTrace();
	...
	//update the selectedSharedPlans
	Vector<Boolean> selectedSharedPlans = new Vector<Boolean>();
	
	
	return rewrittenMapReducePlan;
}
public MROperPlan runQueryDiscoverSharing(){
	
	if(discoveredPlans==null){
		discoveredPlans = new MultiMap<String, MapReduceOper>();
	}
	updatedMapReducePlan= MROperPlanRewriteUtils.rewriteToInjectStores(updatedMapReducePlan, pigContext,discoveredPlans);
	
	return updatedMapReducePlan;
}

public int runQueryExecuteMR(){
	int rc=1;
	if(physicalPlan!=null && mapReducePlan!=null){
	    ...
	    try{  
	    	int results[] = grunt.execQueryFinalize();
		    rc = getReturnCodeForStats(results);
		   gruntCalled=true;
...
public PigStats runQueryExecuteUpdatedMR(){
	int rc=1;
	PigStats stats=null;
	
	//compile the plans if they are null
	if(physicalPlan==null){
		runQueryCompilePP();
		runQueryCompileMRP();
	}
	...
	if(updatedMapReducePlan==null){
		if(mapReducePlan!=null){
			updatedMapReducePlan=this.mapReducePlan.clone();
		}
	}
	
	if(physicalPlan!=null && mapReducePlan!=null && updatedMapReducePlan !=null){
	    ...
	    try{  
	    	//int results[] = grunt.execQueryFinalize(updatedMapReducePlan);
	    	stats =  grunt.execQueryFinalize(updatedMapReducePlan);
		    //rc = getReturnCodeForStats(results);
		   gruntCalled=true;
	...
	//now for every discovered plan, update its information with the stats and store it into the file
	try {
		MROperPlanRewriteUtils.updateRepository(stats, discoveredPlans, sharedPlans, selectedSharedPlans, pigContext);
	} catch (VisitorException e) {
...
public String getPigQueryFileName() {
	return pigQueryFileName;
}


public void setPigQueryFileName(String pigQueryFileName) {
	if(pigQueryFileName!=null){
		this.pigQueryFileName = new String(pigQueryFileName);
	}
}
